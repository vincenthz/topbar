-- copied from taffybar, and tweaked
--
...
verticalBarSetTooltip :: VerticalBarHandle -> Markup -> IO ()
verticalBarSetTooltip (VBH mv) markup = do
    s <- readMVar mv
    case barIsBootstrapped s of
        False -> return ()
        True  -> set (barCanvas s) [ widgetTooltipMarkup := Just markup ]
...
{-  
  setSourceRGB 0 1 1
  setLineWidth 1.0
  moveTo (activeWidth / 2) newOrigin
  lineTo (activeWidth / 2) (newOrigin + 8)
  stroke
-}
